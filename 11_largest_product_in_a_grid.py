...
def horiz(grid):
  ...
  for i in range(len(grid)):
    for j in range(len(grid[i]) - 3):
      check_list = list([grid[i][j],grid[i][j+1],grid[i][j+2],grid[i][j+3]])
      if 0 in check_list:
        continue
      ...
      if check > biggest:
        biggest_list = check_list
        biggest = check
  
  return {"biggest": biggest, "biggest_list": biggest_list}

#horiz_result = horiz(grid)
#print(str(horiz_result))
...
def vert(grid):
  ...
  for i in range(len(grid) - 3):
    for j in range(len(grid[i])):
      check_list = list([grid[i][j],grid[i+1][j],grid[i+2][j],grid[i+3][j]])
      if 0 in check_list:
        continue
...
#vert_result = vert(grid)
#print(str(vert_result))
...
def diag_right(grid):
  ...
  for i in range(len(grid) - 3):
    for j in range(len(grid[i]) - 3):
      check_list = list([grid[i][j],grid[i+1][j+1],grid[i+2][j+2],grid[i+3][j+3]])
      if 0 in check_list:
        continue
...
#diag_right_result = diag_right(grid)
#print(str(diag_right_result))


# largest diag left (cheating)
def diag_left(grid):
  rev_grid = list(grid)
  for item in rev_grid:
    item.reverse()
  return diag_right(rev_grid)


#diag_left_result = diag_left(grid)
#print(str(diag_left_result))
...
print('horizontal:     ' + str(horiz_result))
print('vertical:       ' + str(vert_result))
print('diagonal right: ' + str(diag_right_result))
print('diagonal left:  ' + str(diag_left_result))
